...
def test_git_banner_state_rejects_stale_origin_after_configured_repo_remap(
    ...
    def fake_run(cmd, **kwargs):
        ...
        if joined == "git remote set-url origin https://github.com/DangLemon/hermes-agent.git":
    ...
    assert "git remote set-url origin https://github.com/DangLemon/hermes-agent.git" in command_text
    ...
    assert command_text.index("git update-ref -d refs/remotes/origin/main") < command_text.index(
        "git remote set-url origin https://github.com/DangLemon/hermes-agent.git"
    )
    assert command_text.index("git remote set-url origin https://github.com/DangLemon/hermes-agent.git") < command_text.index(
...
def test_git_banner_state_falls_back_when_stale_origin_invalidation_fails(
    ...
    command_text = [" ".join(str(part) for part in command) for command in commands]
    assert "git update-ref -d refs/remotes/origin/main" in command_text
    assert "git remote set-url origin https://github.com/DangLemon/hermes-agent.git" not in command_text
...
def test_origin_normalization_blocks_matching_fast_path_until_invalidation_finishes(
    ...
    current_origin = {"value": "https://github.com/NousResearch/hermes-agent.git"}
    commands = []
    ...
    def fake_run(cmd, **kwargs):
        ...
        if joined == "git remote set-url origin https://github.com/DangLemon/hermes-agent.git":
            current_origin["value"] = "https://github.com/DangLemon/hermes-agent.git"
            return MagicMock(returncode=0, stdout="")
        if joined == "git show-ref --verify --quiet refs/remotes/origin/main":
            return MagicMock(returncode=0, stdout="")
        if joined == "git update-ref -d refs/remotes/origin/main":
