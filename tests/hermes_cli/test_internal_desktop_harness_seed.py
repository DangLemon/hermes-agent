...
def _seed(tmp_path: Path, home: Path, profile: str = ""):
    ...
    return seed_from_resource(_resource(tmp_path), hermes_home=home, profile=profile)
...
def test_existing_company_row_is_user_owned_and_not_overwritten(tmp_path, monkeypatch):
    ...
    (home / "config.yaml").write_text(
        ...
        "    model: edited-model\n"
        "    key_env: HERMES_CUSTOM_ENDPOINT_LEMON_AI_COMPANY_API_KEY\n"
        "model:\n"
        "  provider: lemon-ai-company\n"
    ...
    assert saved["model"]["key_env"] == "HERMES_CUSTOM_ENDPOINT_LEMON_AI_COMPANY_API_KEY"
...
def test_restart_with_new_managed_dir_preserves_user_selected_endpoint(tmp_path, monkeypatch):
    ...
    from hermes_cli.web_routers.config_env import _write_custom_endpoint, save_config
    from hermes_cli import config, managed_scope
    ...
    monkeypatch.setenv("HERMES_MANAGED_DIR", str(managed_two))
    config._LOAD_CONFIG_CACHE.clear()
    config._RAW_CONFIG_CACHE.clear()
    managed_scope.invalidate_managed_cache()
...
def test_custom_endpoint_validation_uses_saved_profile_key_for_blank_draft(tmp_path, monkeypatch):
    ...
    from hermes_cli.profiles import create_profile
    from hermes_cli.web_models import CustomEndpointUpdate
    from hermes_cli.web_routers.config_env import _config_profile_scope, _custom_endpoint_validation_key
    ...
    with _config_profile_scope("sales"):
        save_env_value("HERMES_CUSTOM_ENDPOINT_OFFICE_AI_API_KEY", "saved-profile-secret")
        save_config(
            {
                "providers": {
                    "office-ai": {
                        ...
                        "base_url": "https://office.example/v1",
                        "model": "office-model",
                        "key_env": "HERMES_CUSTOM_ENDPOINT_OFFICE_AI_API_KEY",
                    }
                }
            }
        )
...
def test_seed_save_merges_concurrent_user_config_writes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("hermes_cli.config.read_user_config_raw", read_then_simulate_concurrent_write)

    result = module.seed_from_resource(_resource(tmp_path), hermes_home=home)
    saved = _read_yaml(home / "config.yaml")
...
from hermes_cli.config import config_file_lock, save_config
    ...
    save_config({
        'providers': {
            'lemon-ai-company': {
                ...
                'key_env': 'HERMES_CUSTOM_COMPANY_KEY',
            }
        }
    }, merge_existing=True)
...
def test_config_write_transaction_allows_reentrant_save_config(tmp_path, monkeypatch):
    from hermes_cli.config import config_write_transaction, save_config
...
def test_config_file_lock_reentrancy_is_keyed_by_config_path(tmp_path, monkeypatch):
    ...
    from hermes_cli.config import config_file_lock
