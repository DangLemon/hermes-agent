...
9. **Adds `%LOCALAPPDATA%\hermes\bin` to User PATH and sets `HERMES_HOME=%LOCALAPPDATA%\hermes`** — exposes the `hermes` command (and points it at your data dir) after you open a new terminal. Only the `hermes.cmd` / `hermes-acp.cmd` wrappers are placed in this `bin` directory; each wrapper binds its installation's update repository before delegating to the venv launcher. The full `venv\Scripts` directory is deliberately **not** placed on PATH so Hermes never shadows your own `python` command.
