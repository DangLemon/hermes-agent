...
const LEMON_MAC_COPY = {
  ...
  NSCalendarsFullAccessUsageDescription: 'Lemon AI needs full access to Calendar to read and manage events when explicitly requested.',
  NSRemindersUsageDescription: 'Lemon AI needs access to Reminders to provide requested personal-assistant and scheduling support.',
  NSRemindersFullAccessUsageDescription: 'Lemon AI needs full access to Reminders to read and manage reminders when explicitly requested.',
  NSScreenCaptureUsageDescription: 'Lemon AI captures the screen when you ask the agent to screenshot or record it.',
  NSLocalNetworkUsageDescription: 'Lemon AI connects to devices on your local network when a plugin or feature you enable requests it.',
...
export function createDesktopPackageConfig(baseBuild, { env = process.env, harnessResource } = {}) {
  ...
  config.extraResources = config.extraResources.map(entry =>
    entry?.from === 'assets/icon.ico' && entry?.to === 'icon.ico'
      ? { ...entry, from: 'assets/lemon-icon.ico' }
      : entry
  )
  config.mac = {
    ...config.mac,
